...
{
    public class NotesRepository : INotesRepo,ISearch<Note>
    {
        /// <summary>
        ...
        public IEnumerable<Note> Search(string term)
        {
            using(var context = new NotesDbContext())
            {
                IEnumerable<Note> lstNotes = context.Notes.Where(n => n.Title.ToLower().Contains(term.ToLower()) 
                                             || n.NoteContent.ToLower().Contains(term.ToLower())).ToList() ;
                return lstNotes;
            }
        }
    }
